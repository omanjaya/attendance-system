  ...
  generateRecommendations() {
    ...
    if (largeComponents.length > 0) {
      this.results.recommendations.push({
        ...
        items: largeComponents.map(
          ([name, data]) => `${name} (${(data.size / 1024).toFixed(1)}KB, ${data.lines} lines)`
        )
      })
    }
    ...
    if (complexComponents.length > 0) {
      this.results.recommendations.push({
        ...
        items: complexComponents.map(([name, data]) => `${name} (complexity: ${data.complexity})`)
      })
    }

    // Heavy dependencies
    const heavyComposables = Object.entries(this.results.composables).filter(
      ([_, data]) => data.dependencies.length > 5
    )
    ...
    if (cssAssets && cssAssets.totalSize > 100000) {
      this.results.recommendations.push({
        ...
        items: cssAssets.files.map(file => `${file.name} (${(file.size / 1024).toFixed(1)}KB)`)
      })
    }
  }
